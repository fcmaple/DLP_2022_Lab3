...
public:
	...
	/**
	 * set a 4-bit tile i = space number, t = tile value
	 */
	void set(int i, int t) { raw = (raw & ~(0x0fULL << (i << 2))) | (uint64_t(t & 0x0f) << (i << 2)); }
	//                             (exclude the space want to fill )|(value write to the space) 
	//刪除 tile
...
private:
	...
	struct lookup {
		...
		void init(int r) {
			...
			score = mvleft(L);// get left score
			left = ((L[0] << 0) | (L[1] << 4) | (L[2] << 8) | (L[3] << 12));

			score = mvleft(R); std::reverse(R, R + 4); //same score whether left and right
		...
		static const lookup& find(int row) {
			static const lookup cache[65536]; //set up all board left right 
			//該row 的所有數字組合 向右向左都考慮好了
			return cache[row]; //mean the cache[row] put the left right board in 
		}
	};
...
public:
	 ...
	 */
	void popup() {
		int space[16], num = 0; //16 space  
		for (int i = 0; i < 16; i++)
			if (at(i) == 0) {  //check if zero or not
				space[num++] = i;
			}
		if (num) // if there  are zero space then rand to pick a space then set one or two
	...
	int move(int opcode) {
		...
		default: return -1;// return reward for one action
		}
	}
	...
	int move_left() {
		...
		raw = move;
		return (move != prev) ? score : -1; //if the same means the action don't change the score and state
	}
	int move_right() {
		uint64_t move = 0;
		uint64_t prev = raw;
	...
	int check_space(){
    	int space_num = 0;
    	for(int i=0;i<16;i++){
        	if(((uint64_t)0xf<<((uint64_t)i<<2) & raw) ==0){
            	space_num++;
        	}
    	}
		return space_num;
	...
	int find_one(){
		int space_num = 1;
		for(int i=0;i<16;i++){
			if(!((uint64_t)0xe<<((uint64_t)i<<2) & raw) && ((uint64_t)0x1<<((uint64_t)i<<2) & raw)){
				space_num=i;
			}
		}
		//if(space_num==-1) info<<"error";
		return space_num;
	}
	//void set(int i, int t) { raw = (raw & ~(0x0fULL << (i << 2))) | (uint64_t(t & 0x0f) << (i << 2)); }
	//                             (exclude the space want to fill )|(value write to the space) 
	int settile(int cursor,int value){
		int c = cursor;
		uint64_t mark = 0x000f;
		while(c<16){
			if(mark<<((uint64_t)c<<2)& raw ) c++;
			else{
				raw = (raw & ~(0x0fULL << (c << 2))) | (uint64_t(value & 0x0f) << (c << 2));
				break;
			}
		}
		if(c>=16) c= -1;
	...
	void deltile(int cursor){
		int c= cursor;
		
		raw = (raw & ~(0x0fULL << (c << 2)));
		
	}
	uint64_t rraw() { return raw;}
public:
...
public: // should be implemented
	...
	virtual float getvalue(const board& b) = 0;
	/**
...
public:
	pattern(const std::vector<int>& p, int iso = 8) : feature(1 << (p.size() * 4)), iso_last(iso) {
		// if 6-tuple ----> feature(1<<24) -> one feature there are 24bit weight
		//one pattern  there are 8 isomorphic
		if (p.empty()) {
		...
		for (int i = 0; i < 8; i++) {
			...
			if (i >= 4) idx.mirror();
			idx.rotate(i);
			for (int t : p) { //{0,1,2,3,4,5}
				isomorphic[i].push_back(idx.at(t));// compute the t th space val and put in isomorphic[i]
				// idx.at(0) = 0x0000 find the file val
			}
			// isomorphic[0] = {0x0000,0x0001,0x0002,0x0003,0x0004,0x0005}
			// isomorphic[1] = {0x000c,0x0008,0x0004,0x0000,0x000d,0x0009}...
...
public:
	 ...
	 * estimate the value of a given board
	 */
	virtual float estimate(const board& b) const {//估計在這個board的這個feature的value
		// TODO
		//進來的是after state then need to estimate the s'' value
		//把b 的空洞都補過1 and 2 
		//丟進去算value 之後出來平均
		//b++ b還原
		board bb = b;
		int space_num = bb.check_space();//找出有幾個空格
		if(space_num==0) info<<"zero\n";
		// info<<space_num<<std::endl;
		float value=0;
		
		int cursor = 0;
		for(cursor=0;cursor<16;cursor++){
			cursor = bb.settile(cursor,1);
			if(cursor==-1) break;
			for(int j=0;j<iso_last;j++){//把每一個isomorphic 的val加起來   n-tuple
				size_t index = indexof(isomorphic[j],bb);
		...
		float value2=0;
		for(cursor=0;cursor<16;cursor++){
			cursor = bb.settile(cursor,2);
			if(cursor==-1) break;
			for(int j=0;j<iso_last;j++){//把每一個isomorphic 的val加起來   n-tuple
				size_t index = indexof(isomorphic[j],bb);
				value2 +=operator[](index);
			}
			bb.deltile(cursor);
		}
		value2/=space_num;
		...
		//index = 0x372135
		//value += weight[0x372135]
	}

	/**
	 * update the value of a given board, and return its updated value
	 * value 平均分給每一個isomorphic 的圖形
	 * 最後回傳所有isomorphic 的value
	 */
	virtual float update(const board& b, float u) {
		//info<<"in update\n";
		// TODO
		//把一個1 關掉 其他0變成1  平均分配error
		// board bb = b;
		// int rand_before_num = bb.check_space()+1;
		// int one_position = bb.find_one();
		// bb.deltile(one_position);
		// float u_split = u/rand_before_num;
		// u_split =  u_split/iso_last; // u = add_value
		float u_split = u/iso_last;
		float value =0;
		// int cursor = 0;
		// if(rand_before_num==1){
		// 	for(cursor=0;cursor<16;cursor++){
		// 		cursor = bb.settile(cursor,1);
		// 		if(cursor==-1) break;
		// 		for(int j=0;j<iso_last;j++){//把每一個isomorphic 的val加起來   n-tuple
		...
		for(int i=0;i<iso_last;i++){
			size_t index = indexof(isomorphic[i],b); //find the weight value of index
			operator[](index) += u_split; //e.g weight[0x372135] += u_split
			value +=operator[](index); //summation of all value of index
		}

		//info<<value<<std::endl;
		return value; // 8 isomorphic value
	}
	float getvalue(const board& b){
		float value =0;
		for(int i=0;i<iso_last;i++){
	...
	//isomorphic[0] = {0,1,2,3,4,5}
	//to_string -> 6 +'-tuple pattern'+ '012345'
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		...
		// patt = {0,1,2,3,4,5}
		// index |= b.at(patt[0])<<(4*0)  --> 1
		// index |= b.at(patt[1])<<(4*1)  --> 3
		// ...
		// index = 0x532731
	}
	...
	std::string nameof(const std::vector<int>& patt) const {
		std::stringstream ss;
		ss << std::hex; //pass hex 
		std::copy(patt.cbegin(), patt.cend(), std::ostream_iterator<int>(ss, ""));//put patt all element to ss and "" between two element
		return ss.str();
	}
	//patt = {0,1,2,3,4,5}
	//ss.str() = "012345"
...
public:
	...
	bool assign(const board& b) {
		...
		after = before = b;
		score = after.move(opcode);//after移動 且回傳reward
		esti = score;
		//v(s) = v(s+1) - v(s)
		return score != -1;
	}
...
public:
	 ...
	 */
	// add_feature(new pattern({0,1,2,3,4,5}))
	void add_feature(feature* feat) {
		...
		info << std::endl;
		//print the pattern's memory size
	}
	 ...
	 */
	float estimate(const board& b) const {//該board的value = all pattern 的value 總和
		//在該處轉換成s'' 並送出去看value
		debug << "estimate " << std::endl << b;
		float value = 0;
		for (feature* feat : feats) {
			float f = feat->estimate(b);
			// info<<f<<" "<<getvalue(b)<<std::endl;
			value += f;
		}
		return value;
	}
	 ...
	 */
	float getvalue(const board& b) const{
		float value=0;
		for(feature* feat:feats){
			value += feat->getvalue(b);
		}
		return value;
	}
	...
	state select_best_move(const board& b) const {
		state after[4] = { 0, 1, 2, 3 }; // up, right, down, left init the opcode
		state* best = after;
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				//暫時改變狀態
				//int space_num = move->after_state().check_space(); //return space number
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for (path.pop_back() /* terminal state */; path.size(); path.pop_back()) {
			...
			float error = move.reward()+exact-getvalue(move.before_state());
			//move.value();
			// info<<"get = "<<getvalue(move.before_state())<<std::endl;
			// info<<move.value()<<std::endl;
	 ...
	 */
	void writetofile(int score , FILE* fp){

		
	}
	void make_statistic(size_t n, const board& b, int score, int unit , FILE* fp) {
		fprintf(fp,"%d ",score);
...
int main(int argc, const char* argv[]) {
	...
	for (size_t n = 1; n <= total; n++) {
		...
		tdl.make_statistic(n, b, score,1000,fp);
		path.clear();
		if(n%100000==0)	tdl.save("0.025_500000");
	}
	fclose(fp);
	// store the model into file
	//tdl.save("0.025_100000");
